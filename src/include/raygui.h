...
RAYGUIAPI int GuiTextBox_old(Rectangle bounds, char *text, int textSize, bool editMode);                   // AMOGUS SUGUMA SUS
        ...
        (wrapMode == TEXT_WRAP_NONE))               // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyPressed(KEY_V))
            {
                ...
                {
                    int clipboardLength = (int)strlen(clipboardText);
                    // Ensure we don't overflow the text buffer
                    if ((textLength + clipboardLength) < textSize)
                    {
                        // Move existing text forward to make space
                        for (int i = textLength + clipboardLength; i > textBoxCursorIndex; i--)
                            text[i] = text[i - clipboardLength];

                        // Insert clipboard text at cursor position
                        for (int i = 0; i < clipboardLength; i++)
                            text[textBoxCursorIndex + i] = clipboardText[i];

                        textBoxCursorIndex += clipboardLength;
...
int GuiTextBox_old(Rectangle bounds, char *text, int textSize, bool editMode)
{
    #if !defined(RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)
        #define RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN  40        // Frames to wait for autocursor movement
    #endif
